...
Claude Code でランブックや実装を Codex で監査すると、ラウンドごとに finding が小出しに出続ける「**五月雨**」現象が起きやすい。本来 1-2 ラウンドで PASS すべきものが 4-13 ラウンド回り続ける。
...
| 範囲 | 階層 | カバー |
|---|---|---|
| 五月雨防止プロンプト v2 (4 行ブロック + 蓄積リスト) / Fix 1, 3, 4, 6 / 収束判定 | **コア (フレームワーク中立)** | 全プロジェクト |
| Fix 2 (DOM canonical baseline) | コア | Web UI / DOM 検証を含むプロジェクト |
| Fix 5 (dotenv 4 load slot + 6 表記) / docs/06 (dev bypass 4 原則) | **Adapter: Next.js + 自前 auth** | Next.js プロジェクト |
| docs/05 14 項目のうち #5/#7/#8/#9/#12 | Adapter: Next.js + Codex CLI | Next.js + Codex 利用者 |
| Rails / Django / Go / 他 framework | **未整備 (Adapter 募集中)** | community contribution 待ち |
...
他フレームワーク / 他コードベースでの汎化は要検証。実走で効いた本物のプロンプト構造は [`docs/02-anti-drip-prompt-v2.md`](docs/02-anti-drip-prompt-v2.md) 参照。なお v1 で推奨していた「重圧プロンプト」(全行 enumerate / mandatory セクション / 3 段階構造 / 全カテゴリ均等深さ) は Codex を tool ループに陥れることが判明したため廃止しました ([`docs/08-known-pitfalls.md`](docs/08-known-pitfalls.md))。

## Self-audit history (本リポ自身を本ツールキットで監査した記録)

| ラウンド | commit | 結果 | 反映 |
|---|---|---|---|
| R1 (4 ファイル並列) | `cbd18c6` → `e5f2f41` | P1×1 + P2×4 = 5 件 | 全件反映 |
| R2 / R1 拡張 (4 ファイル並列) | `e5f2f41` → `a325fcf` | P1×0 + P2×12 + P3×5 = 17 件 | 全件反映 |
| R1 (10 ファイル並列) | `a325fcf` → `7355d55` | P1×5 + P2×39 + P3×18 = 62 件 | 全件反映 |
| R2 (10 ファイル並列) | `7355d55` → `b92b35f` | P1×4 + P2×33 + P3×21 = 58 件 | 全件反映 |
| video-subtitler 知見反映 | `b92b35f` → `aa323c1` | (Fix 6 + 2 系統分岐 追加) | - |
| 構造再編 v0.4-beta | `aa323c1` → 本 commit | (コア / adapter 分離) | - |

R1=5 → R2 拡張=17 → R1 (10 ファイル)=62 → R2=58 と、対象範囲を広げた R1 (10 ファイル) で件数が爆発し R2 で減衰。video-subtitler 知見反映と構造再編は Codex 監査ではなく外部からの input。次の R3 (10 ファイル) で構造再編後の整合を検証する余地あり。
