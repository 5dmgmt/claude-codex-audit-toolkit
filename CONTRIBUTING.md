...
- v1.0 への到達条件: (1) 5dmgmt 系列以外の事例 1+ 件、(2) `docs/_review-notes-v0.4-beta-frozen.md` の各項目を外部事例の知見で再判定して **P1 残ゼロ** にする、(3) 自己監査 / 追加事例で **同一監査単位 (ランブック=同一ファイル / コードベース=同一 scope) + 同一軸換算の下降傾向** を維持、(4) 英訳 README。次の自己監査 (R7) は外部事例で軸が変わってから。
