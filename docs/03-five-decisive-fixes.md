...
## Fix 1. commit pin + 4 placeholder + `git show __SHA__:__RUNBOOK_PATH__`
...
`scripts/codex-audit-prompt-gen.sh` を推奨経路として使う:
...
このスクリプトは内部で:
- `sed '/^##/d'` でテンプレ冒頭の `##` コメント行を除去 (送信本文への汚染防止)
- 4 placeholder (`__SHA__` / `__RUNBOOK_NAME__` / `__RUNBOOK_PATH__` / `__PREV_FINDINGS__`) を bash parameter expansion で置換
...
ランブックに **実効値の優先順位 (process.env 含む)** + 4 ファイル + 6 表記を明記:
...
4. `.env.$NODE_ENV` — 環境別 (`.env.development` / `.env.production` / `.env.test`)
5. `.env` — 全環境 default

検査時は **process.env を最優先で確認 + 4 ファイル全て確認 + 優先順位の重複排除を追跡**。
...
- Workshop Course 1: R1 で 40 件のうち、多くが fix 1-5 で防げる種類の defect
...
- [ ] DOM API は `element.innerText` を canonical baseline として明記 (実行コンテキスト: Playwright Chromium で page load 後取得など)
...
- [ ] dotenv は実効値優先順位 (process.env 最優先) + 4 ファイル + 6 表記すべて検査対象に明記
...
| Fix 5 dotenv 全表記 | 14 項目 #8 (dotenv 4 ファイル) を実効値優先順位含めて拡張 |
