...
| **R7-R12** | (該当なし) | 増減を挟みつつ低位推移 (詳細は Phase C runbook 参照) |
| **最終ラウンド** | R4 で scope cut | R13 で ALL PASS |
| **v2 + 5 fixes 導入** | R2 で部分適用、R3 で完全適用 | R4 で v2 導入、R10-R12 で bypass 4 原則を後追い反映 |
...
両方とも「R3 → R4 で件数が下げ止まった」ようにも読めますが、本質的な差は:

- **Workshop**: R3=6 → R4=6 の **同数停滞** + 反映済方針への別角度指摘 + ランブック膨張限界 → stop
- **SIFT**: R3=18 → R4=6 → R5=5 の **下降傾向継続** + 具体実装の未反映が残っていた → continue
...
4. **ランブック / 実装の物理的制約**: Workshop は受講者が読める範囲を超えていた、SIFT はまだ実装の隙があった
