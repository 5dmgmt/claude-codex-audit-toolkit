...
R1 (40 件) から R2 (13 件) への減少は、**五月雨防止 v2 の部分適用が一因と見られる**。具体例 (当時ログ由来 / runbook には未記録):
- 「同型の問題が 5 箇所にある」を 1 件としてまとめていた指摘 → 5 件として全箇所列挙されたので、R1 の 1 件 → R2 で 0 件に減った (網羅されたので再指摘されない)
- 「角度を変えた言い換え」が 3-4 件あったのが消えた

これらは v2 の 4 行ブロック + 蓄積リストが効いた可能性が高いが、当時のラウンドログから 1:1 の因果を復元するのは困難。
...
これらを根拠に [`04-convergence-patterns.md`](../docs/04-convergence-patterns.md) の収束判定 4 条件と照合:
- **条件 1 (下降傾向)**: R3=6 → R4=6 で非下降 → ❌
- **条件 2 (矛盾指摘なし)**: ✅
- **条件 3 (同一 finding 再発なし)**: 反映済方針への別角度再発あり → ❌
- **条件 4 (具体行に紐づく)**: detail 細分化要求が増えていた → 部分的 ❌
- 加えて修正の負債化サイン: ランブックが想定読者の読める範囲を超えていた

複数条件が ❌ となったため、**今回は scope cut が妥当と判断**。R5 を回さずに v3.4 で確定終了しました。
